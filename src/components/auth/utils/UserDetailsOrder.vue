  ...
  <div>
    <v-row>
      <v-col cols="12" sm="12" md="12">
        <h1 class="text-title text-capitalize" style="color: #424242">
          Historial de Compra
        </h1>
      </v-col>
      <v-col cols="12" md="12">
        <div class="font-weight-bold" style="font-size: 20px; color: #424242">
          Detalles
        </div>
        <v-divider></v-divider>
        <div>
        ...
        <v-divider></v-divider>
        <div v-if="(orderData.buyer = !null)">
          <span class="font-weight-bold">Nombre:</span>
          {{ orderData.buyer.first_name }} {{ orderData.buyer.last_name }}
        </div>
        <div v-if="(orderData.shipping = !null)">
          <span class="font-weight-bold">Direccion de envio:</span>
          {{ orderData.shipping.address_receive }}
        </div>
        ...
        >
          ...
          <template v-slot:[`item.brand`]="{ item }">
            <span v-if="item.product != null" class="text-uppercase">{{
