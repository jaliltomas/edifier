export const isValidUmbral = function ({ zipCode, dataProduct }) {
    ...
    if (userZipCode === 2000) {
        const warehouse = userFindWarehouse({ cp_central: 5, cp_regional: 10, productWarehouse });
        return validateStock({ warehouse, threshold, cp_regional: 10, productWarehouse });
    }

    if (userZipCode === 5000) {
        const warehouse = userFindWarehouse({ cp_central: 5, cp_regional: 3, productWarehouse });
        return validateStock({ warehouse, threshold, cp_regional: 3, productWarehouse });
    }

    return defaultStock({ productWarehouse, threshold });
};

const userFindWarehouse = function ({ cp_central, cp_regional, productWarehouse }) {
...
const defaultStock = function ({ productWarehouse, threshold }) {
    const warehouse = productWarehouse.filter((whr) => whr.warehouse_id == 5 && whr.current_stock > 0);
